...
impl OptimizerState {
    ...
    /// Standard SGD update
    fn update_sgd(&self, gradients: &[Vec<Vec<f64>>]) -> Vec<Vec<Vec<f64>>> {
        gradients
            .iter()
            .map(|layer_grad| {
                layer_grad
...
pub fn train_network(
    network: &mut super::neural_network::NeuralNetwork,
    train_data: &[(Vec<f64>, Vec<f64>)],
    config: &TrainingConfig,
) -> super::TrainingHistory {
    ...
    for epoch in 0..config.epochs {
        ...
        for batch in train_data.chunks(config.batch_size) {
            ...
            for (input, target) in batch {
                ...
                epoch_loss += super::neural_network::compute_loss(output, target, config.loss_function);
