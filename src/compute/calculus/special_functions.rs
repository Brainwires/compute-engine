...
pub fn riemann_zeta(s: f64) -> f64 {
    ...
    let correction = 1.0 / (2.0 * n.powf(s)) + s / (12.0 * n.powf(s + 1.0))
...
pub fn handle_riemann_zeta(params: &HashMap<String, Value>) -> SpecialFunctionsResult {
    let s = params.get("s").and_then(|v| v.as_f64()).unwrap_or(2.0);
...
pub fn handle_elliptic_integral(params: &HashMap<String, Value>) -> SpecialFunctionsResult {
    let k = params.get("k").and_then(|v| v.as_f64()).unwrap_or(0.5);
    let kind = params
        .get("type")
        .and_then(|v| v.as_str())
        .unwrap_or("first");
