...
pub fn black_scholes_monte_carlo(
    ...
    let option_value = (payoff_sum / n_simulations as f64) * (-risk_free_rate * time_to_expiry).exp();
    option_value
}
...
pub fn handle_ito_integral(params: &HashMap<String, Value>) -> StochasticCalculusResult {
    let integrand_values: Vec<f64> = params.get("integrand_values")
...
pub fn handle_sde_solution(params: &HashMap<String, Value>) -> StochasticCalculusResult {
    let sde_type = params.get("sde_type").and_then(|v| v.as_str()).unwrap_or("geometric_brownian");
    let initial_value = params.get("initial_value").and_then(|v| v.as_f64()).unwrap_or(1.0);
    let t_max = params.get("t_max").and_then(|v| v.as_f64()).unwrap_or(1.0);
    let n_steps = params.get("n_steps").and_then(|v| v.as_u64()).unwrap_or(1000) as usize;
    let method = params.get("method").and_then(|v| v.as_str()).unwrap_or("euler_maruyama");
    
    let solution = match (sde_type, method) {
        ("geometric_brownian", "euler_maruyama") => {
            solve_sde_euler_maruyama(geometric_drift, geometric_diffusion, initial_value, t_max, n_steps)
        },
        ("geometric_brownian", "milstein") => {
            solve_sde_milstein(geometric_drift, geometric_diffusion, geometric_diffusion_derivative, initial_value, t_max, n_steps)
        },
        ...
        ("mean_reverting", "euler_maruyama") => {
            solve_sde_euler_maruyama(linear_drift, constant_diffusion, initial_value, t_max, n_steps)
        },
        _ => {
            solve_sde_euler_maruyama(geometric_drift, geometric_diffusion, initial_value, t_max, n_steps)
        },
...
pub fn handle_stochastic_calculus_ops(params: &HashMap<String, Value>) -> StochasticCalculusResult {
    let operation = params.get("operation").and_then(|v| v.as_str()).unwrap_or("brownian_motion");
    
    match operation {
        "brownian_motion" => {
            let t_max = params.get("t_max").and_then(|v| v.as_f64()).unwrap_or(1.0);
            let n_steps = params.get("n_steps").and_then(|v| v.as_u64()).unwrap_or(1000) as usize;
            let initial_value = params.get("initial_value").and_then(|v| v.as_f64()).unwrap_or(0.0);
            let drift = params.get("drift").and_then(|v| v.as_f64()).unwrap_or(0.0);
            let volatility = params.get("volatility").and_then(|v| v.as_f64()).unwrap_or(1.0);
        ...
        "black_scholes_monte_carlo" => {
            let spot_price = params.get("spot_price").and_then(|v| v.as_f64()).unwrap_or(100.0);
            let strike_price = params.get("strike_price").and_then(|v| v.as_f64()).unwrap_or(100.0);
            let risk_free_rate = params.get("risk_free_rate").and_then(|v| v.as_f64()).unwrap_or(0.05);
            let volatility = params.get("volatility").and_then(|v| v.as_f64()).unwrap_or(0.2);
            let time_to_expiry = params.get("time_to_expiry").and_then(|v| v.as_f64()).unwrap_or(1.0);
            let n_simulations = params.get("n_simulations").and_then(|v| v.as_u64()).unwrap_or(10000) as usize;
            let option_type = params.get("option_type").and_then(|v| v.as_str()).unwrap_or("call");
            
            let option_value = black_scholes_monte_carlo(
                spot_price, strike_price, risk_free_rate, volatility, 
                time_to_expiry, n_simulations, option_type
            );
        ...
        _ => StochasticCalculusResult {
            ...
            result: None,
            error: Some(format!("Unknown stochastic calculus operation: {}", operation)),
        }
    }
}