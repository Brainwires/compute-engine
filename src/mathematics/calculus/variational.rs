...
pub fn compute_euler_lagrange_equation(lagrangian: &str) -> String {
    ...
    match lagrangian {
        ...
        "sqrt(1 + y'^2)" | "sqrt(1+y'^2)" => {
            "y'' / (1 + y'^2)^(3/2) = 0".to_string()
        },
        "0.5*m*y'^2 - V(y)" => {
            "m*y'' + dV/dy = 0".to_string()
        },
        _ => {
...
pub fn compute_action_functional(
    lagrangian_values: &[f64],
    dx: f64,
) -> f64 {
...
pub fn handle_euler_lagrange(params: &HashMap<String, Value>) -> VariationalCalculusResult {
    let lagrangian = params.get("lagrangian")
        .and_then(|v| v.as_str())
        .unwrap_or("0.5*y'^2");
    ...
    match method {
        "euler_lagrange" => {
            ...
            let solution = if let Some(bc) = params.get("boundary_conditions") {
                ...
                Some(solve_euler_lagrange_bvp(lagrangian, (x0, x1), (y0, y1), 100))
            } else {
                None
            };
        ...
        "hamilton_principle" => {
            ...
            let y0 = params.get("boundary_conditions")
                .and_then(|bc| bc.get("y0"))
                .and_then(|v| v.as_f64())
                .unwrap_or(0.0);
            let y1 = params.get("boundary_conditions")
            ...
            let solution = solve_euler_lagrange_bvp(
                lagrangian, x_range, (y0, y1), num_points
            );
