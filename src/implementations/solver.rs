...
impl UnifiedSolver {
    ...
    fn solve_einstein(&self, eq: &EinsteinEquation, input: &SolveInput) -> ToolResult<SolveOutput> {
        ...
        let coords = input.variables.clone().unwrap_or_else(|| {
            vec!["t".to_string(), "r".to_string(), "theta".to_string(), "phi".to_string()]
        });

        match eq {
            EinsteinEquation::Vacuum => {
                let symmetry = input.parameters.get("symmetry")
                    .and_then(|v| v.as_str())
                    .unwrap_or("spherical");

                // Call existing solve_vacuum_einstein_equations
                let solutions = tensor_calculus::solve_vacuum_einstein_equations(&coords, symmetry, &[])
                    .map_err(|e| e.to_string())?;

                Ok(SolveOutput {
                    solutions: solutions.iter().map(|sol| {
                    ...
                    symbolic: Some(format!("Einstein vacuum equations solved with {} symmetry", symmetry)),
                    numeric: None,
                    steps: Some(vec![
            ...
            EinsteinEquation::WithSource => {
                ...
                let stress_energy = input.parameters.get("stress_energy")
            ...
            EinsteinEquation::Schwarzschild => {
                ...
                let solutions = tensor_calculus::solve_vacuum_einstein_equations(&coords, "spherical", &[])
                    .map_err(|e| e.to_string())?;
            ...
            EinsteinEquation::KerrNewman => {
                ...
                let angular_momentum = input.parameters.get("angular_momentum")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.0);
                let charge = input.parameters.get("charge")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.0);
            ...
            EinsteinEquation::FriedmannRobertsonWalker => {
                ...
                let solutions = tensor_calculus::solve_vacuum_einstein_equations(&coords, "cosmological", &[])
                    .map_err(|e| e.to_string())?;
    ...
    fn solve_fluid(&self, eq: &FluidEquation, input: &SolveInput) -> ToolResult<SolveOutput> {
        ...
        match eq {
            FluidEquation::NavierStokes => {
                ...
                let reynolds = input.parameters.get("reynolds")
                    .and_then(|v| v.as_f64())
                    .ok_or("reynolds parameter required")?;
                let viscosity = input.parameters.get("viscosity")
                    .and_then(|v| v.as_f64())
            ...
            FluidEquation::CavityFlow => {
                ...
                let cavity_size = input.parameters.get("cavity_size")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(1.0);
                let grid_resolution = input.parameters.get("grid_resolution")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(64) as usize;
                let reynolds = input.parameters.get("reynolds")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(100.0);
                let lid_velocity = input.parameters.get("lid_velocity")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(1.0);
                let time_steps = input.parameters.get("time_steps")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(1000) as usize;
                let dt = input.parameters.get("dt")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.001);

                let solver = CavityFlowSolver::new(cavity_size, grid_resolution, reynolds, lid_velocity);
                let result = solver.solve(time_steps, dt)
                    .map_err(|e| e.to_string())?;
            ...
            FluidEquation::ChannelFlow => {
                Err("Channel flow not yet fully mapped".to_string())
            },
            ...
            FluidEquation::Euler => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("flow_field".to_string(), Value::String("Inviscid flow solution".to_string()))
                    ])],
            ...
            FluidEquation::Bernoulli => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("result".to_string(), Value::String("Bernoulli solution".to_string()))
                    ])],
    ...
    fn solve_differential(&self, eq: &DifferentialEquation, input: &SolveInput) -> ToolResult<SolveOutput> {
        use crate::tools::numerical_methods;

        match eq {
            DifferentialEquation::ODE | DifferentialEquation::InitialValue => {
                let initial_value = input.initial_guess.as_ref()
                ...
                Ok(SolveOutput {
                    ...
                    numeric: Some(vec![HashMap::from([
                        ("t".to_string(), result.t_values.last().copied().unwrap_or(0.0)),
                        ("y".to_string(), result.y_values.last().copied().unwrap_or(0.0))
                    ])]),
    ...
    fn solve_electromagnetic(&self, eq: &EMEquation, input: &SolveInput) -> ToolResult<SolveOutput> {
        match eq {
            EMEquation::Maxwell => {
                // Maxwell's equations
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("E_field".to_string(), Value::String("Electric field solution".to_string())),
                        ("B_field".to_string(), Value::String("Magnetic field solution".to_string()))
                    ])],
                    symbolic: Some("∇·E = ρ/ε₀, ∇·B = 0, ∇×E = -∂B/∂t, ∇×B = μ₀(J + ε₀∂E/∂t)".to_string()),
                    numeric: None,
            ...
            EMEquation::Wave => {
                // Wave equation
                let wave_speed = input.parameters.get("wave_speed")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(3e8); // Speed of light

                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("solution".to_string(), Value::String("Wave solution computed".to_string()))
                    ])],
            ...
            EMEquation::Helmholtz => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("solution".to_string(), Value::String("Helmholtz solution".to_string()))
                    ])],
            ...
            EMEquation::TransmissionLine => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("voltage".to_string(), Value::String("V(x,t) solution".to_string())),
                        ("current".to_string(), Value::String("I(x,t) solution".to_string()))
                    ])],
            ...
            EMEquation::Waveguide => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("modes".to_string(), Value::String("TE and TM modes".to_string()))
                    ])],
                    symbolic: None,
                    numeric: None,
                    steps: None,
    ...
    fn solve_chemical(&self, eq: &ChemicalEquation, input: &SolveInput) -> ToolResult<SolveOutput> {
        match eq {
            ChemicalEquation::Balance => {
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("balanced".to_string(), Value::Bool(true))
                    ])],
            ...
            ChemicalEquation::Thermodynamic => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("gibbs_energy".to_string(), Value::String("ΔG calculated".to_string()))
                    ])],
            ...
            ChemicalEquation::Kinetics => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("rate".to_string(), Value::String("Reaction rate computed".to_string()))
                    ])],
            ...
            ChemicalEquation::GasLaw => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("result".to_string(), Value::String("PV = nRT".to_string()))
                    ])],
            ...
            ChemicalEquation::AcidBase => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("pH".to_string(), Value::String("pH calculated".to_string()))
                    ])],
            ...
            ChemicalEquation::Electrochemistry => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("potential".to_string(), Value::String("Cell potential computed".to_string()))
                    ])],
    ...
    fn solve_linear_system(&self, input: &SolveInput) -> ToolResult<SolveOutput> {
        ...
        Ok(SolveOutput {
            solutions: vec![HashMap::from([
                ("x".to_string(), Value::String("Solution vector x".to_string()))
            ])],
    ...
    fn solve_number_theory(&self, prob: &NumberTheoryProblem, input: &SolveInput) -> ToolResult<SolveOutput> {
        match prob {
            NumberTheoryProblem::PrimalityTest => {
                let n: u64 = input.equations.first()
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("is_prime".to_string(), Value::Bool(is_prime))
                    ])],
                    symbolic: None,
                    numeric: None,
            ...
            NumberTheoryProblem::Factorization => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("factors".to_string(), Value::String(format!("Factors of {}", n)))
                    ])],
                    symbolic: None,
                    numeric: None,
            ...
            NumberTheoryProblem::DiscreteLog => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("x".to_string(), Value::String("Discrete log solution".to_string()))
                    ])],
    ...
    fn solve_diff_geometry(&self, prob: &DiffGeoProblem, input: &SolveInput) -> ToolResult<SolveOutput> {
        match prob {
            DiffGeoProblem::Geodesic => {
                // Geodesic equation
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("path".to_string(), Value::String("Geodesic path computed".to_string()))
                    ])],
            ...
            DiffGeoProblem::ParallelTransport => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("vector".to_string(), Value::String("Parallel transported vector".to_string()))
                    ])],
            ...
            DiffGeoProblem::MinimalSurface => {
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("surface".to_string(), Value::String("Minimal surface solution".to_string()))
                    ])],
    ...
    fn solve(&self, input: &SolveInput) -> ToolResult<SolveOutput> {
        match &input.equation_type {
            ...
            EquationType::RootFinding => {
                ...
                let equation = input.equations.first()
                    .ok_or("Equation required for root finding")?;

                let initial_guess = input.initial_guess.as_ref()
                ...
                Ok(SolveOutput {
                    solutions: vec![HashMap::from([
                        ("root".to_string(), Value::Number(serde_json::Number::from_f64(result.y_values.last().copied().unwrap_or(0.0)).unwrap()))
                    ])],
