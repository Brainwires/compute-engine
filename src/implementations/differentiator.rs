...
impl UnifiedDifferentiator {
    ...
    fn differentiate_vector_calc(&self, op: &VectorCalcOp, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        ...
        match op {
            VectorCalcOp::Gradient => {
                ...
                let gradient_components: Vec<String> = gradient_results.iter()
                    ...
                    .collect();
                derivatives.insert("gradient".to_string(), serde_json::json!(gradient_components));
            ...
            VectorCalcOp::Curl => {
                ...
                let mut derivatives = std::collections::HashMap::new();
                derivatives.insert("curl".to_string(), serde_json::json!(vec![curl_x.clone(), curl_y.clone(), curl_z.clone()]));
                ...
                Ok(DifferentiateOutput {
                    derivatives,
                    latex: Some(std::collections::HashMap::from([
                        ("curl".to_string(), format!("\\nabla \\times \\vec{{F}} = ({}, {}, {})", curl_x, curl_y, curl_z))
                    ])),
                    metadata: Some(serde_json::json!({
                        "operation": "curl",
                        "method": "symbolic",
            ...
            VectorCalcOp::Directional => {
                ...
                let direction: Vec<f64> = input.parameters.get("direction")
                    .and_then(|v| serde_json::from_value(v.clone()).ok())
                    .ok_or("Directional derivative requires 'direction' vector parameter")?;

                if direction.len() != input.variables.len() {
                    return Err(format!("Direction vector length {} must match variables {}", direction.len(), input.variables.len()));
                }
                ...
                let mut derivatives = std::collections::HashMap::new();
                derivatives.insert("directional".to_string(), serde_json::json!(directional_deriv));

                Ok(DifferentiateOutput {
                    derivatives,
                    latex: Some(std::collections::HashMap::from([
                        ("directional".to_string(), format!("D_{{\\vec{{v}}}} f = {}", directional_deriv))
                    ])),
                    metadata: Some(serde_json::json!({
    ...
    fn differentiate_tensor_calc(&self, op: &TensorDiffOp, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        use crate::mathematics::symbolic_cas;
        ...
        match op {
            TensorDiffOp::Covariant => {
                ...
                let metric_exprs: Vec<Vec<symbolic_cas::Expr>> = metric_data.iter()
                ...
                let tensor_components: Vec<String> = input.parameters.get("tensor_components")
                    .and_then(|v| serde_json::from_value(v.clone()).ok())
                ...
                for mu in 0..dimension {
                    for nu in 0..dimension {
                        if nu < tensor_components.len() {
                            ...
                            let partial = symbolic_cas::differentiate(&tensor_components[nu], coords[mu], None)
                            ...
                            for lambda in 0..dimension {
                                if lambda < tensor_components.len() {
                                    let gamma = christoffel.get(&[nu, mu, lambda])
                                        .map_err(|e| format!("Failed to get Christoffel symbol: {}", e))?;
                                    ...
                                    // Γ^ν_μλ * V^λ
                                    if gamma_str != "0" {
                                        connection_terms.push(format!("({}) * ({})", gamma_str, component));
                                    }
                                }
                            }
                ...
                let full_derivs: Vec<String> = covariant_derivs.iter()
                    .map(|(_, _, d)| d.clone())
                    .collect();
            ...
            TensorDiffOp::Lie => {
                ...
                let vector_field: Vec<String> = input.parameters.get("vector_field")
                    .and_then(|v| serde_json::from_value(v.clone()).ok())
                    .ok_or("Lie derivative requires 'vector_field' parameter [X^0, X^1, ...]")?;

                if vector_field.len() != input.variables.len() {
                    return Err(format!("Vector field dimension {} must match variables {}",
                        vector_field.len(), input.variables.len()));
                }

                // Check if we're operating on a scalar or vector
                let target_components: Option<Vec<String>> = input.parameters.get("target_components")
                    .and_then(|v| serde_json::from_value(v.clone()).ok());
                ...
                match target_components {
                    None => {
                        ...
                        Ok(DifferentiateOutput {
                            derivatives,
                            latex: Some(std::collections::HashMap::from([
                                ("lie".to_string(), format!("\\mathcal{{L}}_X f = X^\\mu \\partial_\\mu f = {}", lie_deriv))
                            ])),
                            metadata: Some(serde_json::json!({
                    ...
                    Some(target) => {
                        ...
                        for mu in 0..dimension {
                            ...
                            for (nu, var) in input.variables.iter().enumerate() {
                                ...
                                first_terms.push(format!("({}) * ({})", vector_field[nu], partial_y.expression));
                            }

                            // Second term: -Y^ν ∂_ν X^μ (sum over ν)
                            let mut second_terms = Vec::new();
                            for (nu, var) in input.variables.iter().enumerate() {
                                let partial_x = symbolic_cas::differentiate(&vector_field[mu], var, None)
                                    .map_err(|e| format!("Failed to compute ∂_ν X^μ: {}", e))?;

                                second_terms.push(format!("({}) * ({})", target[nu], partial_x.expression));
                            }
                            ...
                            derivatives.insert(format!("L_X Y^{}", mu), serde_json::json!(lie_component));
                        }
            ...
            TensorDiffOp::ExteriorDerivative => {
                ...
                let form_degree = input.parameters.get("form_degree")
                    .and_then(|v| v.as_u64())
                ...
                match form_degree {
                    0 => {
                        ...
                        derivatives.insert("exterior".to_string(), serde_json::json!(exterior_deriv));

                        Ok(DifferentiateOutput {
                            derivatives,
                            latex: Some(std::collections::HashMap::from([
                                ("exterior".to_string(), format!("df = \\sum_i \\frac{{\\partial f}}{{\\partial x_i}} dx_i"))
                            ])),
                            metadata: Some(serde_json::json!({
                    ...
                    1 => {
                        ...
                        for i in 0..n {
                            for j in (i+1)..n {
                                if i < form_components.len() && j < form_components.len() {
                                    ...
                                    let wedge_coeff = format!("({}) - ({})", df_j_dx_i.expression, df_i_dx_j.expression);
                                    let wedge_term = format!("({}) d{} ∧ d{}", wedge_coeff, input.variables[i], input.variables[j]);
                        ...
                        Ok(DifferentiateOutput {
                            derivatives,
                            latex: Some(std::collections::HashMap::from([
                                ("exterior".to_string(), format!("d\\omega = \\sum_{{i<j}} \\left(\\frac{{\\partial f_j}}{{\\partial x_i}} - \\frac{{\\partial f_i}}{{\\partial x_j}}\\right) dx_i \\wedge dx_j"))
                            ])),
                            metadata: Some(serde_json::json!({
                                "operation": "exterior_derivative",
                                "form_degree": 1,
                    ...
                    _ => {
                        ...
                        Ok(DifferentiateOutput {
                            derivatives,
                            latex: Some(std::collections::HashMap::from([
                                ("exterior".to_string(), format!("d(\\omega_{}) \\text{{ where }} \\omega \\text{{ is a {}-form}}", form_degree, form_degree))
                            ])),
                            metadata: Some(serde_json::json!({
                                "operation": "exterior_derivative",
    ...
    fn differentiate_numeric(&self, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        ...
        let x_values: Vec<f64> = input.parameters.get("x_values")
            .and_then(|v| serde_json::from_value(v.clone()).ok())
            .ok_or("x_values required for numeric differentiation")?;

        let y_values: Vec<f64> = input.parameters.get("y_values")
            .and_then(|v| serde_json::from_value(v.clone()).ok())
            .ok_or("y_values required for numeric differentiation")?;

        let order = input.order.as_ref()
            .and_then(|orders| orders.first())
            .copied()
            .unwrap_or(1);

        let method = input.parameters.get("method")
            .and_then(|v| v.as_str())
            .unwrap_or("central");
        ...
        let mut derivatives = std::collections::HashMap::new();
        let var_name = input.variables.first()
        ...
        Ok(DifferentiateOutput {
            derivatives,
            latex: Some(std::collections::HashMap::from([
                (var_name, format!("d{}/dx", input.expression))
            ])),
            metadata: Some(serde_json::json!({
    ...
    fn differentiate_symbolic(&self, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        use crate::mathematics::symbolic_cas;
        ...
        Ok(DifferentiateOutput {
            derivatives,
            latex: if latex_map.is_empty() { None } else { Some(latex_map) },
            metadata: Some(serde_json::json!({
    ...
    fn differentiate_variational(&self, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        use crate::mathematics::symbolic_cas;
        ...
        let independent_var = input.parameters.get("independent_var")
            .and_then(|v| v.as_str())
            .unwrap_or("x");

        // Get the dependent variable (typically 'y')
        let dependent_var = input.parameters.get("dependent_var")
            .and_then(|v| v.as_str())
            .unwrap_or("y");

        // Derivative variable (typically 'y_prime' or 'dy')
        let derivative_var = input.parameters.get("derivative_var")
            .and_then(|v| v.as_str())
            .unwrap_or("y_prime");
        ...
        derivatives.insert("∂L/∂y".to_string(), serde_json::json!(d_l_d_y.expression.clone()));

        // Compute ∂L/∂y'
        let d_l_d_yprime = symbolic_cas::differentiate(&lagrangian, derivative_var, None)
            .map_err(|e| format!("Failed to compute ∂L/∂y': {}", e))?;

        derivatives.insert("∂L/∂y'".to_string(), serde_json::json!(d_l_d_yprime.expression.clone()));
        ...
        let d_dx_partial = symbolic_cas::differentiate(&d_l_d_yprime.expression, independent_var, None);
        ...
        derivatives.insert("d/dx(∂L/∂y')".to_string(), serde_json::json!(d_dx_term.clone()));

        // Form the Euler-Lagrange equation: d/dx(∂L/∂y') - ∂L/∂y = 0
        let euler_lagrange = format!("({}) - ({}) = 0", d_dx_term, d_l_d_y.expression);
        derivatives.insert("euler_lagrange".to_string(), serde_json::json!(euler_lagrange.clone()));
        ...
        // ∂²L/∂y'²
        let d2_l_d_yprime2 = symbolic_cas::differentiate(&d_l_d_yprime.expression, derivative_var, None);
        if let Ok(result) = d2_l_d_yprime2 {
            derivatives.insert("∂²L/∂y'²".to_string(), serde_json::json!(result.expression));
        }

        // ∂²L/∂y∂y' (mixed partial)
        let d2_l_d_y_d_yprime = symbolic_cas::differentiate(&d_l_d_y.expression, derivative_var, None);
        if let Ok(result) = d2_l_d_y_d_yprime {
            derivatives.insert("∂²L/∂y∂y'".to_string(), serde_json::json!(result.expression));
        }

        Ok(DifferentiateOutput {
            derivatives,
            latex: Some(std::collections::HashMap::from([
                ("euler_lagrange".to_string(), format!(
                    "\\frac{{d}}{{d{}}}\\left(\\frac{{\\partial L}}{{\\partial y'}}\\right) - \\frac{{\\partial L}}{{\\partial {}}} = 0",
                    independent_var, dependent_var
                )),
                ("∂L/∂y".to_string(), format!("\\frac{{\\partial L}}{{\\partial {}}} = {}", dependent_var, d_l_d_y.expression)),
                ("∂L/∂y'".to_string(), format!("\\frac{{\\partial L}}{{\\partial y'}} = {}", d_l_d_yprime.expression))
            ])),
            metadata: Some(serde_json::json!({
    ...
    fn differentiate_forms(&self, input: &DifferentiateInput) -> ToolResult<DifferentiateOutput> {
        ...
        if form_degree == 0 && !input.parameters.contains_key("form_components") {
            ...
            return self.differentiate_tensor_calc(&TensorDiffOp::ExteriorDerivative, &modified_input);
        }
        ...
        if form_degree >= 1 {
            ...
            let exterior_deriv = format!("d({})", input.expression);
            derivatives.insert("exterior_derivative".to_string(), serde_json::json!(exterior_deriv.clone()));

            return Ok(DifferentiateOutput {
                derivatives,
                latex: Some(std::collections::HashMap::from([
                    ("exterior_derivative".to_string(), format!("d(\\omega) \\text{{ where }} \\omega \\text{{ is a {}-form}}", form_degree))
                ])),
                metadata: Some(serde_json::json!({
        ...
        derivatives.insert("differential".to_string(), serde_json::json!(differential.clone()));
        derivatives.insert("exterior_derivative".to_string(), serde_json::json!(differential.clone()));

        Ok(DifferentiateOutput {
            derivatives,
            latex: Some(std::collections::HashMap::from([
                ("differential".to_string(), format!("df = \\sum_i \\frac{{\\partial f}}{{\\partial x_i}} dx_i"))
            ])),
            metadata: Some(serde_json::json!({
                "operation": "differential_forms",
                "form_degree": 0,
