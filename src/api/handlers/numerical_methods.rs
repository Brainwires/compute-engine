...
pub fn handle(request: &ComputationRequest) -> ComputationResponse {
    let result = match request.operation.as_str() {
        ...
        "differentiate" => {
            let req: DifferentiationRequest = match serde_json::from_value(
        ...
        _ => Err(format!(
            "Unknown numerical_methods operation: {}",
            request.operation
        )),
    };
...
#[cfg(test)]
#[path = "../../../tests/unit/api/handlers/numerical_methods_handler_tests.rs"]
mod tests;
