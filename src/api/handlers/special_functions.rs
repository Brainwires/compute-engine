...
pub fn handle(request: &ComputationRequest) -> ComputationResponse {
    let result = match request.operation.as_str() {
        ...
        "elliptic" => {
            let req: EllipticIntegralRequest = match serde_json::from_value(
        ...
        _ => Err(format!(
            "Unknown special_functions operation: {}",
            request.operation
        )),
    };
...
#[cfg(test)]
#[path = "../../../tests/unit/api/handlers/special_functions_handler_tests.rs"]
mod tests;
