...
pub fn handle(request: &ComputationRequest) -> ComputationResponse {
    let result = match request.operation.as_str() {
        ...
        "transmission_line" => {
            let req: TransmissionLineRequest = match serde_json::from_value(
        ...
        _ => Err(format!(
            "Unknown electromagnetism operation: {}",
            request.operation
        )),
    };
...
#[cfg(test)]
#[path = "../../../tests/unit/api/handlers/electromagnetism_handler_tests.rs"]
mod tests;
