//! Validate Tool
//!
//! Provides equation and physics validation:
//! - Full equation validation
//! - Dimensional consistency checking
//! - Conservation law validation
//! - Symmetry property checking
...
pub use equation::{
    check_conservation_laws, check_dimensional_consistency, check_mathematical_correctness,
    check_physics_compliance, check_symmetries, extract_variables, parse_equation,
    validate_equation, ValidationResult,
};
...
///
/// Routes validation requests to the equation validation module
    ...
    fn validate(&self, input: &ValidateInput) -> ToolResult<ValidateOutput> {
        match &input.operation {
            ValidateOp::Equation => {
                ...
                Ok(ValidateOutput {
                    ...
                    suggestions: if result.violations.is_empty() {
                        ...
                        Some(vec![
                            "Review the violations and correct the equation".to_string(),
                        ])
                    },
            ...
            ValidateOp::Dimensions => {
                ...
                let (consistent, unit_analysis) =
                    check_dimensional_consistency(&input.expression, &units_map)
            ...
            ValidateOp::Conservation => {
                ...
                let violations =
                    check_conservation_laws(&input.expression, domain, &conservation_laws)
            ...
            ValidateOp::Symmetry => {
                ...
                let violations = check_symmetries(&input.expression, domain, &symmetries)
            ...
            ValidateOp::Physics => {
                ...
                let (compliant, violations) =
                    check_physics_compliance(&input.expression, domain)
            ...
            ValidateOp::Bounds => {
                ...
                let is_correct = check_mathematical_correctness(&input.expression)
                ...
                if input.expression.contains("/") {
                    warnings.push(
                        "Expression contains division - ensure denominator is non-zero".to_string(),
                    );
                }
                ...
                if input.expression.contains("log") || input.expression.contains("ln") {
                    warnings.push(
                        "Expression contains logarithm - ensure argument is positive".to_string(),
                    );
                }
                ...
                if input.expression.contains("sqrt") {
                    warnings.push(
                        "Expression contains square root - ensure argument is non-negative"
                            .to_string(),
                    );
                }
            ...
            ValidateOp::Singularities => {
                ...
                // Parse the equation
                let (left, right) = parse_equation(&input.expression)
                ...
                let mut check_singularities = |expr: &str, singularities: &mut Vec<String>| {
                    ...
                    if expr.contains("/x") || expr.contains("/y") || expr.contains("/z") {
                        singularities
                            .push("Potential singularity: division by variable".to_string());
                    }
                    ...
                    if expr.contains("tan(") {
                        singularities.push(
                            "Potential singularity: tan function has poles at odd multiples of pi/2"
                                .to_string(),
                        );
                    }
                    if expr.contains("cot(") {
                        singularities.push(
                            "Potential singularity: cot function has poles at multiples of pi"
                                .to_string(),
                        );
                    }
                    if expr.contains("sec(") {
                        singularities.push(
                            "Potential singularity: sec function has poles at odd multiples of pi/2"
                                .to_string(),
                        );
                    }
                    if expr.contains("csc(") {
                        singularities.push(
                            "Potential singularity: csc function has poles at multiples of pi"
                                .to_string(),
                        );
                    }
                    ...
                    if expr.contains("gamma(") {
                        singularities
                            .push("Gamma function has poles at non-positive integers".to_string());
                    }
                };
