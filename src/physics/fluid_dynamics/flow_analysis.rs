...
impl FlowAnalyzer {
    ...
    pub fn analyze(&self, velocity_field: Value, grid_spacing: Value, analysis_types: Vec<String>) -> Result<Value, Box<dyn Error>> {
        // Parse velocity field
        let u_velocity = velocity_field["u_velocity"].as_array()
            .ok_or("Missing u_velocity field")?;
        let v_velocity = velocity_field["v_velocity"].as_array()
            .ok_or("Missing v_velocity field")?;

        // Parse grid spacing
        let dx = grid_spacing["dx"].as_f64()
            .ok_or("Missing dx in grid_spacing")?;
        let dy = grid_spacing["dy"].as_f64()
            .ok_or("Missing dy in grid_spacing")?;

        // Convert JSON arrays to 2D vectors
        let u: Result<Vec<Vec<f64>>, _> = u_velocity.iter()
            .map(|row| {
                row.as_array()
        ...
        let v: Result<Vec<Vec<f64>>, _> = v_velocity.iter()
            .map(|row| {
                row.as_array()
        ...
        for analysis_type in &analysis_types {
            match analysis_type.as_str() {
                ...
                "kinetic_energy" => {
                    ...
                    results.insert("kinetic_energy".to_string(), json!({
                ...
                _ => {
                    results.insert(analysis_type.clone(), 
    ...
    fn calculate_vorticity(&self, u: &Vec<Vec<f64>>, v: &Vec<Vec<f64>>, dx: f64, dy: f64) -> Result<Vec<Vec<f64>>, Box<dyn Error>> {
        let rows = u.len();
        let cols = u[0].len();
    ...
    fn calculate_divergence(&self, u: &Vec<Vec<f64>>, v: &Vec<Vec<f64>>, dx: f64, dy: f64) -> Result<Vec<Vec<f64>>, Box<dyn Error>> {
        let rows = u.len();
        let cols = u[0].len();
    ...
    fn calculate_stream_function(&self, u: &Vec<Vec<f64>>, v: &Vec<Vec<f64>>, dx: f64, dy: f64) -> Result<Vec<Vec<f64>>, Box<dyn Error>> {
        let rows = u.len();
        let cols = u[0].len();
        ...
        if max_inconsistency > 0.1 {
            eprintln!("Warning: Stream function calculation has high inconsistency: {}", max_inconsistency);
        }

        Ok(psi)
    }

    fn calculate_kinetic_energy(&self, u: &Vec<Vec<f64>>, v: &Vec<Vec<f64>>) -> KineticEnergyResult {
        let rows = u.len();
        let cols = u[0].len();
    ...
    fn calculate_strain_rate(&self, u: &Vec<Vec<f64>>, v: &Vec<Vec<f64>>, dx: f64, dy: f64) -> Result<StrainRateResult, Box<dyn Error>> {
        let rows = u.len();
        let cols = u[0].len();
        ...
        for i in 1..rows-1 {
            for j in 1..cols-1 {
                ...
                magnitude[i][j] = (2.0 * (s11[i][j].powi(2) + s22[i][j].powi(2) + 2.0 * s12[i][j].powi(2))).sqrt();
            }
        }
