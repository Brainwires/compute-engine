    ...
    pub fn solve(&self, time_steps: usize, dt: f64) -> Result<Value, Box<dyn Error>> {
        ...
        for step in 0..time_steps {
            ...
            if step % (time_steps / 10).max(1) == 0 {
                eprintln!(
                    "Cavity flow simulation: {:.1}% complete",
        ...
        let u_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| u[[i, j]]).collect())
            .collect();
        let v_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| v[[i, j]]).collect())
            .collect();
        let p_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| p[[i, j]]).collect())
            .collect();
        let vort_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| vorticity[[i, j]]).collect())
            .collect();
        let stream_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| stream_function[[i, j]]).collect())
            .collect();
        let vel_mag_vec: Vec<Vec<f64>> = (0..n)
            .map(|i| (0..n).map(|j| velocity_magnitude[[i, j]]).collect())
            .collect();
