    ...
    output = match expression.trim() {
        ...
        expr if is_power_of_x(expr, &diff_var) => {
            ...
            if exp == "1" {
                ...
                if n == 2 {
                    steps.push(format!("Applied power rule: d/dx({}^{}) = {}*{}", base, n, n, base));
                    format!("{}*{}", n, base)
                } else {
                    steps.push(format!("Applied power rule: d/dx({}^{}) = {}*{}^{}", base, n, n, base, n-1));
                    if n - 1 == 1 {
                        format!("{}*{}", n, base)
                    } else {
                        format!("{}*{}^{}", n, base, n - 1)
        ...
        expr if expr.starts_with("sin(") => {
            ...
            steps.push(format!("Applied trigonometric rule: d/dx(sin(u)) = cos(u)*du/dx"));
            if inner == diff_var {
                format!("cos({})", inner)
            } else {
        ...
        expr if expr.starts_with("cos(") => {
            let inner = extract_function_argument(expr);
            steps.push(format!("Applied trigonometric rule: d/dx(cos(u)) = -sin(u)*du/dx"));
            if inner == diff_var {
                format!("-sin({})", inner)
            } else {
        ...
        expr if expr.starts_with("exp(") => {
            let inner = extract_function_argument(expr);
            steps.push(format!("Applied exponential rule: d/dx(exp(u)) = exp(u)*du/dx"));
            if inner == diff_var {
                format!("exp({})", inner)
            } else {
        ...
        expr if expr.starts_with("log(") || expr.starts_with("ln(") => {
            let inner = extract_function_argument(expr);
            steps.push(format!("Applied logarithmic rule: d/dx(ln(u)) = (1/u)*du/dx"));
            if inner == diff_var {
                format!("1/{}", inner)
            } else {
...
pub fn symbolic_integrate(
    ...
    output = match expression.trim() {
        ...
        expr if is_power_of_x(expr, &int_var) => {
            ...
            if let Ok(n) = exp.parse::<i32>() {
                if n == -1 {
                    ...
                    let new_exp = n + 1;
                    steps.push(format!("Applied power rule: ∫x^{} dx = x^{}/{} + C", n, new_exp, new_exp));
        ...
        // Default case
        _ => {
            steps.push("Complex expression - integration may require advanced techniques".to_string());
...
fn is_constant(expr: &str, var: &str) -> bool {
    !expr.contains(var) && expr.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-')
}
...
fn analyze_expression_complexity(input: &str, output: &str) -> ComplexityAnalysis {
    ...
    let function_types = vec![
        if input.contains("sin") || input.contains("cos") { Some("trigonometric") } else { None },
        if input.contains("exp") || input.contains("ln") { Some("exponential") } else { None },
        if input.contains("^") { Some("polynomial") } else { None },
