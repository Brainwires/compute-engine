...
pub fn simulate_particle_interactions(
    ...
    let result = match interaction_type {
        ...
        _ => return Err(anyhow::anyhow!("Unknown interaction type: {}", interaction_type)),
    };
    
    let computation_time = start.elapsed().as_millis();
    eprintln!("✅ Particle simulation completed in {} ms", computation_time);
...
fn simulate_electromagnetic_interaction(
    ...
    let charged_particles: Vec<_> = particles.iter()
        .filter(|p| p.charge.abs() > 1e-10)
        .cloned()
        .collect();
    
    if charged_particles.len() < 2 {
        physical_insights.push("Need at least 2 charged particles for electromagnetic interaction".to_string());
    }
    ...
    if has_photon_and_electron(&particles) {
        ...
        physical_insights.push("Energy and momentum conservation determine scattering angles".to_string());
    ...
    if has_electron_positron_pair(&particles) {
        ...
        particles.push(create_photon(total_energy / 2.0, Vector3D { x: 1.0, y: 0.0, z: 0.0 }));
        particles.push(create_photon(total_energy / 2.0, Vector3D { x: -1.0, y: 0.0, z: 0.0 }));
    }
    ...
    for step in 0..steps.min(10) {
        simulation_steps.push(SimulationStep {
            ...
            particles_state: particles.iter().map(|p| ParticleState {
    ...
    let feynman_diagrams = vec![
        FeynmanDiagram {
            diagram_id: "qed_vertex".to_string(),
            interaction_vertices: vec![
                InteractionVertex {
            ...
            external_lines: vec![],
            loop_order: 0,
            coupling_constants: [("alpha".to_string(), 1.0/137.0)].iter().cloned().collect(),
        }
    ];
...
fn simulate_weak_interaction(
    ...
    if has_neutron(&particles) {
        ...
        if let Some(neutron_idx) = particles.iter().position(|p| matches!(p.particle_type, ParticleType::Neutron)) {
    ...
    let decay_channels = vec![
        DecayChannel {
            channel_id: "beta_decay".to_string(),
            products: vec!["proton".to_string(), "electron".to_string(), "electron_antineutrino".to_string()],
            branching_ratio: 1.0,
            q_value: 0.782, // MeV
...
fn simulate_higgs_interaction(
    ...
    let has_higgs = particles.iter().any(|p| matches!(p.particle_type, ParticleType::Higgs));
...
fn simulate_particle_decay(
    ...
    Ok(InteractionResult {
        ...
        feynman_diagrams: vec![],
        simulation_steps: vec![],
        physical_insights: vec!["Unstable particles decay according to their lifetimes".to_string()],
    })
}
...
fn has_photon_and_electron(particles: &[Particle]) -> bool {
    let has_photon = particles.iter().any(|p| matches!(p.particle_type, ParticleType::Photon));
    let has_electron = particles.iter().any(|p| matches!(p.particle_type, ParticleType::Electron));
    has_photon && has_electron
}

fn has_electron_positron_pair(particles: &[Particle]) -> bool {
    let has_electron = particles.iter().any(|p| matches!(p.particle_type, ParticleType::Electron) && p.charge < 0.0);
    let has_positron = particles.iter().any(|p| matches!(p.particle_type, ParticleType::Electron) && p.charge > 0.0);
    has_electron && has_positron
}

fn has_neutron(particles: &[Particle]) -> bool {
    particles.iter().any(|p| matches!(p.particle_type, ParticleType::Neutron))
}
...
fn is_unstable(particle_type: &ParticleType) -> bool {
    matches!(particle_type, 
        ParticleType::Neutron | ParticleType::Muon | ParticleType::Tau |
        ParticleType::Pion | ParticleType::Kaon | ParticleType::Higgs
    )
}
...
fn get_decay_products(particle_type: &ParticleType) -> Vec<Particle> {
    match particle_type {
        ParticleType::Neutron => vec![create_proton(), create_electron(), create_electron_antineutrino()],
        ParticleType::Muon => vec![create_electron(), create_muon_neutrino(), create_electron_antineutrino()],
...
fn get_decay_product_names(particle_type: &ParticleType) -> Vec<String> {
    match particle_type {
        ParticleType::Neutron => vec!["proton".to_string(), "electron".to_string(), "electron_antineutrino".to_string()],
        ParticleType::Muon => vec!["electron".to_string(), "muon_neutrino".to_string(), "electron_antineutrino".to_string()],
        ParticleType::Pion => vec!["muon".to_string(), "muon_neutrino".to_string()],
        _ => vec![],
    }
}

fn analyze_conservation_laws(initial: &[Particle], final_particles: &[Particle]) -> ConservationAnalysis {
...
fn calculate_total_quantum_numbers(particles: &[Particle]) -> QuantumNumbers {
    QuantumNumbers {
        baryon_number: particles.iter().map(|p| p.quantum_numbers.baryon_number).sum(),
        lepton_number: particles.iter().map(|p| p.quantum_numbers.lepton_number).sum(),
        strangeness: particles.iter().map(|p| p.quantum_numbers.strangeness).sum(),
...
fn create_photon(energy: f64, direction: Vector3D) -> Particle {
    Particle {
        ...
        momentum: Vector3D { x: direction.x * energy, y: direction.y * energy, z: direction.z * energy },
        energy,
        lifetime: None,
        quantum_numbers: QuantumNumbers {
            baryon_number: 0.0, lepton_number: 0.0, strangeness: 0.0,
            charm: 0.0, beauty: 0.0, truth: 0.0, isospin: 0.0, color_charge: None,
        },
    }
}
...
fn create_proton() -> Particle {
    Particle {
        ...
        spin: 0.5,
        position: Vector3D { x: 0.0, y: 0.0, z: 0.0 },
        momentum: Vector3D { x: 0.0, y: 0.0, z: 0.0 },
        energy: 0.938272,
        lifetime: None, // Stable
        quantum_numbers: QuantumNumbers {
            baryon_number: 1.0, lepton_number: 0.0, strangeness: 0.0,
            charm: 0.0, beauty: 0.0, truth: 0.0, isospin: 0.5, color_charge: None,
        },
    }
}
...
fn create_electron() -> Particle {
    Particle {
        ...
        lifetime: None, // Stable
        quantum_numbers: QuantumNumbers {
            baryon_number: 0.0, lepton_number: 1.0, strangeness: 0.0,
            charm: 0.0, beauty: 0.0, truth: 0.0, isospin: 0.0, color_charge: None,
        },
    }
}
...
fn create_pion() -> Particle {
    Particle {
        ...
        quantum_numbers: QuantumNumbers {
            baryon_number: 0.0, lepton_number: 0.0, strangeness: 0.0,
            charm: 0.0, beauty: 0.0, truth: 0.0, isospin: 1.0, color_charge: None,
        },
    }
}