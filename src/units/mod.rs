//! Units Tool
//!
//! Provides unit conversion and dimensional analysis:
//! - Unit conversion between compatible units
//! - Dimensional analysis of expressions
...
pub use dimensional_analysis::{
    are_units_compatible, convert_units, derive_units_for_quantity, dimensional_analysis,
    get_physics_formulas, get_si_base_units, tokenize_unit, DimensionInfo,
    DimensionalAnalysisResult, PhysicalDimension, PhysicsFormula, UnitConversion,
};
...
///
/// Routes unit and dimensional analysis requests to the dimensional_analysis module
    ...
    fn units(&self, input: &UnitsInput) -> ToolResult<UnitsOutput> {
        match &input.operation {
            UnitsOp::Convert => {
                ...
                let result = convert_units(from_unit, to_unit, value)
            ...
            UnitsOp::Analyze => {
                ...
                let result = dimensional_analysis(expression.clone(), variable_units, target)
            ...
            UnitsOp::CheckCompatibility => {
                ...
                let compatible = are_units_compatible(from_unit, to_unit)
            ...
            UnitsOp::GetBase => {
                ...
                let si_base = get_si_base_units(unit)
            ...
            UnitsOp::Derive => {
                ...
                let units = derive_units_for_quantity(quantity)
            ...
            UnitsOp::Parse => {
                ...
                let si_base = get_si_base_units(unit).unwrap_or_else(|_| "unknown".to_string());
            ...
            UnitsOp::Simplify => {
                ...
                let si_base =
                    get_si_base_units(unit).map_err(|e| format!("Simplify failed: {}", e))?;
