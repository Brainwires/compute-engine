...
impl NavierStokesSolver {
    ...
    pub fn solve(&self) -> Result<Value, Box<dyn Error>> {
        ...
        let fluid_props = self.config["fluid_properties"]
            .as_object()
            .ok_or("Missing fluid properties")?;
        let boundary_conds = self.config["boundary_conditions"]
            .as_object()
            .ok_or("Missing boundary conditions")?;
        let initial_conds = self.config["initial_conditions"]
            .as_object()
            .ok_or("Missing initial conditions")?;
        let sim_params = self.config["simulation_parameters"]
            .as_object()
        ...
        let nx = domain["grid_points_x"]
            .as_u64()
            .ok_or("Invalid grid points x")? as usize;
        let ny = domain["grid_points_y"]
            .as_u64()
            .ok_or("Invalid grid points y")? as usize;

        // Parse fluid properties
        let viscosity = fluid_props["viscosity"]
            .as_f64()
            .ok_or("Invalid viscosity")?;
        let density = fluid_props["density"].as_f64().ok_or("Invalid density")?;

        // Parse simulation parameters
        let dt = sim_params["time_step"]
            .as_f64()
            .ok_or("Invalid time step")?;
        let total_time = sim_params["total_time"]
            .as_f64()
            .ok_or("Invalid total time")?;
        ...
        for step in 0..time_steps {
            ...
            if step % (time_steps / 10).max(1) == 0 {
                eprintln!(
                    "2D Navier-Stokes simulation: {:.1}% complete",
        ...
        let u_vec: Vec<Vec<f64>> = (0..nx)
            .map(|i| (0..ny).map(|j| u[[i, j]]).collect())
            .collect();
        let v_vec: Vec<Vec<f64>> = (0..nx)
            .map(|i| (0..ny).map(|j| v[[i, j]]).collect())
            .collect();
        let p_vec: Vec<Vec<f64>> = (0..nx)
            .map(|i| (0..ny).map(|j| p[[i, j]]).collect())
            .collect();
        let vort_vec: Vec<Vec<f64>> = (0..nx)
            .map(|i| (0..ny).map(|j| vorticity[[i, j]]).collect())
            .collect();
        let div_vec: Vec<Vec<f64>> = (0..nx)
            .map(|i| (0..ny).map(|j| divergence[[i, j]]).collect())
            .collect();
    ...
    fn solve_momentum_step(
        ...
        for i in 1..nx - 1 {
            for j in 1..ny - 1 {
                ...
                u_new[[i, j]] =
                    u[[i, j]] + dt * (-u_dudx - v_dudy - dpdx / rho + nu * (d2udx2 + d2udy2));
            }
        }
        ...
        for i in 1..nx - 1 {
            for j in 1..ny - 1 {
                ...
                v_new[[i, j]] =
                    v[[i, j]] + dt * (-u_dvdx - v_dvdy - dpdy / rho + nu * (d2vdx2 + d2vdy2));
            }
        }
