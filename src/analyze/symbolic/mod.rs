...
//!
//! This module also provides the symbolic operations for the ANALYZE tool.
...
// =============================================================================
// ANALYZE tool handler functions
// =============================================================================

/// Parse and validate expression
pub fn analyze_parse(input: &AnalyzeInput) -> ToolResult<AnalyzeOutput> {
    use crate::validate;

    let result = validate::parse_equation(&input.expression).map_err(|e| e.to_string())?;
...
pub fn analyze_simplify(input: &AnalyzeInput) -> ToolResult<AnalyzeOutput> {
...
pub fn analyze_extract_variables(input: &AnalyzeInput) -> ToolResult<AnalyzeOutput> {
    use crate::validate;

    let variables = validate::extract_variables(&input.expression);
...
pub fn analyze_partial_fraction(input: &AnalyzeInput) -> ToolResult<AnalyzeOutput> {
    let factored = factor(&input.expression)
        .map_err(|e| format!("Partial fraction factorization failed: {}", e))?;

    let simplified = simplify(&input.expression)
