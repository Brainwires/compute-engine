...
fn test_em_antenna() {
    ...
    if let Ok(ToolResponse::FieldTheory(output)) = result {
        ...
        assert!(field_data.contains_key("frequency"), "Should have frequency");
        assert!(field_data.contains_key("wavelength"), "Should have wavelength");
    }
}
...
fn test_em_waveguide() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute waveguide modes: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert!(field_data.contains_key("cutoff_frequency"), "Should have cutoff frequency");
        assert!(field_data.contains_key("propagating"), "Should indicate if propagating");
    }
}
...
fn test_em_scattering() {
    ...
    params.insert("scattering_angle".to_string(), serde_json::json!(std::f64::consts::PI / 4.0)); // 45 degrees
    ...
    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert!(field_data.contains_key("cross_section"), "Should have cross section");
        assert!(field_data.contains_key("scattering_angle"), "Should have scattering angle");
    }
}
...
fn test_green_function_poisson_3d() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute Poisson Green's function: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert_eq!(field_data.get("equation").and_then(|v| v.as_str()).unwrap(), "poisson");
        assert_eq!(field_data.get("dimension").and_then(|v| v.as_u64()).unwrap(), 3);
        assert!(field_data.contains_key("green_function"), "Should have Green's function");
    }
}
...
fn test_green_function_poisson_2d() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute 2D Poisson Green's function: {:?}", result);
}
...
fn test_green_function_poisson_1d() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute 1D Poisson Green's function: {:?}", result);
}
...
fn test_green_function_helmholtz() {
    ...
    params.insert("wavenumber".to_string(), serde_json::json!(2.0 * std::f64::consts::PI));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute Helmholtz Green's function: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert_eq!(field_data.get("equation").and_then(|v| v.as_str()).unwrap(), "helmholtz");
        assert!(field_data.contains_key("wavenumber"), "Should have wavenumber");
    }
}
...
fn test_green_function_diffusion() {
    ...
    assert!(result.is_err() || result.is_ok(), "Diffusion Green's function test");
}
...
fn test_quantum_scalar_field() {
    ...
    if let Ok(ToolResponse::FieldTheory(output)) = result {
        ...
        assert!(field_data.contains_key("propagator"), "Should have propagator");
    }
}
...
fn test_quantum_dirac_field() {
    ...
    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert!(field_data.contains_key("mass"), "Should have mass");
        assert_eq!(field_data.get("spin").and_then(|v| v.as_f64()).unwrap(), 0.5, "Should be spin-1/2");
        assert_eq!(field_data.get("antiparticle").and_then(|v| v.as_bool()).unwrap(), true, "Should have antiparticle");
    }
}
...
fn test_quantum_gauge_field_qed() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute QED gauge field: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert_eq!(field_data.get("gauge_group").and_then(|v| v.as_str()).unwrap(), "U(1)");
        assert_eq!(field_data.get("massless").and_then(|v| v.as_bool()).unwrap(), true, "Photon should be massless");
    }
}
...
fn test_quantum_gauge_field_qcd() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute QCD gauge field: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        assert_eq!(field_data.get("gauge_group").and_then(|v| v.as_str()).unwrap(), "SU(3)");
    }
}
...
fn test_waveguide_below_cutoff() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute waveguide below cutoff: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        // Below cutoff, wave should not propagate
        let propagating = field_data.get("propagating").and_then(|v| v.as_bool()).unwrap();
...
fn test_antenna_microwave() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute microwave antenna: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let field_data = output.field_values[0].as_object().unwrap();
        let wavelength = field_data.get("wavelength").and_then(|v| v.as_f64()).unwrap();
        // c / f = 3e8 / 10e9 = 0.03m = 3cm
        assert!((wavelength - 0.03).abs() < 0.001, "Wavelength should be ~3cm");
    }
}
...
fn test_scalar_field_klein_gordon() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Should compute Klein-Gordon propagator: {:?}", result);

    if let Ok(ToolResponse::FieldTheory(output)) = result {
        let metadata = output.metadata.as_ref().unwrap();
        assert_eq!(metadata.get("equation").and_then(|v| v.as_str()).unwrap(), "klein_gordon");
    }
}
