...
fn test_schrodinger_infinite_well() {
    ...
    let request = ToolRequest::Compute(ComputeInput {
        operation: ComputeOp::Physics(PhysicsOp::QuantumMechanics(QuantumMechOp::SchrodingerEquation)),
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        let wavefunction = output.result.get("wavefunction").and_then(|v| v.as_f64()).unwrap();
...
fn test_schrodinger_harmonic_oscillator() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Harmonic oscillator should succeed: {:?}", result);
...
fn test_schrodinger_step_potential() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Step potential should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let wavefunction = output.result.get("wavefunction").and_then(|v| v.as_f64()).unwrap();
        assert!(wavefunction.is_finite(), "Wavefunction should be finite");

        let transmission = output.result.get("transmission_coefficient").and_then(|v| v.as_f64());
        if let Some(t) = transmission {
            assert!(t >= 0.0 && t <= 1.0, "Transmission coefficient should be between 0 and 1");
        }
    }
}
...
fn test_schrodinger_barrier_potential() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Barrier potential should succeed: {:?}", result);
...
fn test_harmonic_oscillator_ground_state() {
    ...
    let request = ToolRequest::Compute(ComputeInput {
        operation: ComputeOp::Physics(PhysicsOp::QuantumMechanics(QuantumMechOp::HarmonicOscillator)),
        data: serde_json::json!({}),
        parameters: params,
    });
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        let energy = output.result.get("energy").and_then(|v| v.as_f64()).unwrap();
        // E_0 = (1/2)ℏω
        let expected_energy = 0.5 * H_BAR * 1.0e15;
        assert!((energy - expected_energy).abs() / expected_energy < 0.01,
                "Ground state energy should be (1/2)ℏω");

        let wavefunction = output.result.get("wavefunction").and_then(|v| v.as_f64()).unwrap();
        assert!(wavefunction > 0.0, "Wavefunction at origin should be positive for ground state");

        let turning_points = output.result.get("classical_turning_points").and_then(|v| v.as_array());
        assert!(turning_points.is_some(), "Should return classical turning points");
    }
}
...
fn test_hydrogen_1s_orbital() {
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        ...
        assert!((energy - expected_energy).abs() / expected_energy.abs() < 0.1,
                "1s energy should be ~-13.6 eV, got {} J", energy);

        let angular_momentum = output.result.get("orbital_angular_momentum").and_then(|v| v.as_f64()).unwrap();
        assert!(angular_momentum.abs() < 1.0e-10, "1s has zero orbital angular momentum (l=0)");
    }
}
...
fn test_hydrogen_2p_orbital() {
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        ...
        assert!(angular_momentum > 0.0, "2p should have non-zero orbital angular momentum (l=1)");
    }
}
...
fn test_hydrogen_invalid_quantum_numbers() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_err(), "Should reject invalid quantum numbers (l >= n)");
}
...
fn test_angular_momentum_eigenvalues() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Angular momentum should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let total = output.result.get("total_angular_momentum").and_then(|v| v.as_f64()).unwrap();
        // L_total = √(l(l+1))ℏ = √(2*3)ℏ = √6 ℏ for l=2
        let expected_total = (6.0_f64).sqrt() * H_BAR;
        assert!((total - expected_total).abs() / expected_total < 0.01,
                "Total angular momentum should be √(l(l+1))ℏ");

        let z_component = output.result.get("z_component").and_then(|v| v.as_f64()).unwrap();
        // L_z = mℏ = 1ℏ
        let expected_z = H_BAR;
        assert!((z_component - expected_z).abs() / expected_z < 0.01,
...
fn test_angular_momentum_ladder_operators() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Ladder operators should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let ladder_result = output.result.get("ladder_result").and_then(|v| v.as_f64());
        assert!(ladder_result.is_some(), "Should return ladder operator coefficient");

        // For l=1, m=0, L+|1,0⟩ = ℏ√(l(l+1) - m(m+1))|1,1⟩ = ℏ√(2 - 0)
        let coeff = ladder_result.unwrap();
        let expected = H_BAR * (2.0_f64).sqrt();
        assert!((coeff - expected).abs() / expected < 0.01, "L+ coefficient should be ℏ√2");
    }
}
...
fn test_spin_pauli_matrices() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Spin operators should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let eigenvalues = output.result.get("eigenvalues").and_then(|v| v.as_array()).unwrap();
...
fn test_spin_expectation_values() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Spin expectation should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let expectation = output.result.get("expectation_value").and_then(|v| v.as_f64());
        if let Some(exp_val) = expectation {
            // ⟨S_z⟩ for |↑⟩ should be +ℏ/2
            let expected = H_BAR / 2.0;
            assert!((exp_val - expected).abs() / expected < 0.01,
...
fn test_perturbation_first_order() {
    ...
    params.insert("perturbation_matrix_element".to_string(), serde_json::json!(0.1)); // ⟨n|H'|n⟩
    params.insert("unperturbed_energy".to_string(), serde_json::json!(1.0));

    let request = ToolRequest::Compute(ComputeInput {
        operation: ComputeOp::Physics(PhysicsOp::QuantumMechanics(QuantumMechOp::PerturbationTheory)),
        data: serde_json::json!({}),
        parameters: params,
    });

    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "First-order perturbation should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let energy_correction = output.result.get("energy_correction").and_then(|v| v.as_f64()).unwrap();
        assert!(energy_correction.is_finite(), "Energy correction should be finite");
    }
}
...
fn test_perturbation_second_order() {
    ...
    params.insert("perturbation_matrix_element".to_string(), serde_json::json!(0.0)); // ⟨n|H'|n⟩ = 0 for 2nd order
    params.insert("unperturbed_energy".to_string(), serde_json::json!(1.0));
    params.insert("energy_differences".to_string(), serde_json::json!([1.0, 0.5]));
    params.insert("coupling_matrix_elements".to_string(), serde_json::json!([0.1, 0.15]));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Second-order perturbation should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let energy_correction = output.result.get("energy_correction").and_then(|v| v.as_f64()).unwrap();
        assert!(energy_correction.is_finite(), "Second-order correction should be finite");
    }
}
...
fn test_tunneling_under_barrier() {
    ...
    let request = ToolRequest::Compute(ComputeInput {
        operation: ComputeOp::Physics(PhysicsOp::QuantumMechanics(QuantumMechOp::TunnelingProbability)),
        data: serde_json::json!({}),
        parameters: params,
    });
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        let transmission = output.result.get("transmission_coefficient").and_then(|v| v.as_f64()).unwrap();
        assert!(transmission >= 0.0 && transmission <= 1.0,
                "Transmission coefficient should be between 0 and 1");
        assert!(transmission < 0.5, "Tunneling probability should be small for E < V₀");

        let reflection = output.result.get("reflection_coefficient").and_then(|v| v.as_f64()).unwrap();
        assert!((transmission + reflection - 1.0).abs() < 0.01,
                "T + R should equal 1 (probability conservation)");

        let decay_constant = output.result.get("decay_constant").and_then(|v| v.as_f64()).unwrap();
        assert!(decay_constant > 0.0, "Decay constant should be positive for E < V₀");
    }
}
...
fn test_tunneling_over_barrier() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Over-barrier transmission should succeed: {:?}", result);
...
fn test_density_matrix_pure_state() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Pure state density matrix should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let purity = output.result.get("purity").and_then(|v| v.as_f64()).unwrap();
        assert!((purity - 1.0).abs() < 0.01, "Pure state should have purity = 1");

        let entropy = output.result.get("von_neumann_entropy").and_then(|v| v.as_f64()).unwrap();
...
fn test_density_matrix_mixed_state() {
    ...
    params.insert("density_matrix".to_string(), serde_json::json!([[0.5, 0.0], [0.0, 0.5]]));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Mixed state density matrix should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let purity = output.result.get("purity").and_then(|v| v.as_f64()).unwrap();
        assert!(purity < 1.0, "Mixed state should have purity < 1");
        assert!((purity - 0.5).abs() < 0.01, "Maximally mixed 2-level system should have purity = 0.5");
...
fn test_entanglement_bell_state() {
    ...
    params.insert("state_vector".to_string(), serde_json::json!([sqrt2_inv, 0.0, 0.0, sqrt2_inv]));

    let request = ToolRequest::Compute(ComputeInput {
        operation: ComputeOp::Physics(PhysicsOp::QuantumMechanics(QuantumMechOp::EntanglementMeasure)),
        data: serde_json::json!({}),
        parameters: params,
    });

    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Entanglement measure should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let concurrence = output.result.get("entanglement_measure").and_then(|v| v.as_f64()).unwrap();
        assert!(concurrence >= 0.0 && concurrence <= 1.0,
                "Concurrence should be between 0 and 1");
        assert!(concurrence > 0.9, "Bell state should be maximally entangled (C ≈ 1)");
    }
}
...
fn test_entanglement_separable_state() {
    ...
    let mut params = HashMap::new();
    params.insert("measure".to_string(), serde_json::json!("entanglement_entropy"));
    // Separable state |00⟩ - no entanglement
    params.insert("state_vector".to_string(), serde_json::json!([1.0, 0.0, 0.0, 0.0]));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Entanglement entropy should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let entropy = output.result.get("entanglement_measure").and_then(|v| v.as_f64()).unwrap();
        assert!(entropy.abs() < 0.1, "Separable state should have near-zero entanglement entropy");
    }
}
...
fn test_quantum_entropy_von_neumann() {
    ...
    params.insert("density_matrix".to_string(), serde_json::json!([[0.7, 0.0], [0.0, 0.3]]));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "von Neumann entropy should succeed: {:?}", result);
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        ...
        assert!(entropy > 0.5 && entropy < 0.7, "Entropy should be ~0.611 for this state");
    }
}
...
fn test_quantum_coherence_l1_norm() {
    ...
    let mut params = HashMap::new();
    params.insert("density_matrix".to_string(), serde_json::json!([[0.5, 0.3], [0.3, 0.5]]));
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Quantum coherence should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let coherence = output.result.get("l1_coherence").and_then(|v| v.as_f64()).unwrap();
        assert!(coherence >= 0.0, "Coherence should be non-negative");
        // L1 coherence = sum of |off-diagonal elements| = 2*|0.3| = 0.6
        assert!((coherence - 0.6).abs() < 0.01, "L1 coherence should be ~0.6");
    }
}
...
fn test_bell_inequality_chsh() {
    ...
    let sqrt2_inv = 1.0 / (2.0_f64).sqrt();
    params.insert("state_vector".to_string(), serde_json::json!([sqrt2_inv, 0.0, 0.0, sqrt2_inv]));
    params.insert("measurement_angles".to_string(), serde_json::json!([0.0, 0.785, 0.0, 0.785])); // π/4
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Bell inequality test should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let chsh_value = output.result.get("chsh_value").and_then(|v| v.as_f64()).unwrap();
        assert!(chsh_value > 2.0, "Bell state should violate CHSH inequality (S > 2)");
        assert!(chsh_value <= 2.0 * (2.0_f64).sqrt() + 0.1,
                "CHSH value should not exceed Tsirelson bound (~2.828)");

        let violates = output.result.get("violates_inequality").and_then(|v| v.as_bool()).unwrap();
...
fn test_quantum_gates_single_qubit() {
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        let output_state = output.result.get("output_state").and_then(|v| v.as_array()).unwrap();
        ...
        let sqrt2_inv = 1.0 / (2.0_f64).sqrt();
        assert!((amplitude_0 - sqrt2_inv).abs() < 0.01, "H|0⟩ should have equal amplitudes");
        assert!((amplitude_1 - sqrt2_inv).abs() < 0.01, "H|0⟩ should have equal amplitudes");
    }
}
...
fn test_quantum_gates_cnot() {
    ...
    params.insert("input_state".to_string(), serde_json::json!([0.0, 1.0, 0.0, 0.0])); // |01⟩
    ...
    if let Ok(ToolResponse::Compute(output)) = result {
        let output_state = output.result.get("output_state").and_then(|v| v.as_array()).unwrap();
        assert_eq!(output_state.len(), 4, "CNOT output should be 4-component state");

        // CNOT|01⟩ = |01⟩ (control is 0, target unchanged)
        assert!((output_state[1].as_f64().unwrap() - 1.0).abs() < 0.01,
...
fn test_quantum_circuit() {
    ...
    params.insert("gates".to_string(), serde_json::json!([
        {"type": "hadamard", "qubit": 0},
        {"type": "cnot", "control": 0, "target": 1}
    ]));
    params.insert("initial_state".to_string(), serde_json::json!([1.0, 0.0, 0.0, 0.0])); // |00⟩
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_ok(), "Quantum circuit should succeed: {:?}", result);

    if let Ok(ToolResponse::Compute(output)) = result {
        let final_state = output.result.get("final_state").and_then(|v| v.as_array()).unwrap();
        assert_eq!(final_state.len(), 4, "2-qubit state should have 4 components");
        ...
        let norm_sq: f64 = final_state.iter()
...
fn test_density_matrix_requires_valid_input() {
    ...
    let result = dispatcher.dispatch(request);
    assert!(result.is_err(), "Should require state_vector for pure state");
}
